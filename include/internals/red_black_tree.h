...
namespace mstl {
	...
	private:
		...
		std::pair<base_node_type*, bool> insert_impl(U&& v)
		{
			...
			const auto& key_ex = this->m_KeyExtractor;
			const auto& comp = this->m_Comp;
			...
			while (current)
			{
				...
				if (comp(key_ex(v), key_ex(val)))
				{
					current = current->mp_Left;
				}
				else if (comp(key_ex(val), key_ex(v)))
				{
			...
			else if (comp(key_ex(v), key_ex(static_cast<const node_type*>(parent)->m_Val)))
			{
		...
		void inorder_print_rec(base_node_type* node, base_node_type* parent, const char* relation) const noexcept {
			...
			// === Parent ===
			if (p) {
				if constexpr (requires { p->m_Val.first; })
					std::cout << "  Parent: " << p->m_Val.first << "\n";
				else
			...
			// === Left ===
			if (l) {
				if constexpr (requires { l->m_Val.first; })
					std::cout << "  Left: " << l->m_Val.first << "\n";
				else
			...
			// === Right ===
			if (r) {
				if constexpr (requires { r->m_Val.first; })
					std::cout << "  Right: " << r->m_Val.first << "\n";
				else
